...
/*---------------------------------------------*/
/*
const bt_uuid_t APP_BLE_CHAR_UUID128 = BT_UUID_INIT_WITH_UUID16(APP_BLE_CHAR_UUID);

extern uint32_t app_ble_charc_value_callback (const uint8_t rw, uint16_t handle, void *data, uint16_t size, uint16_t offset);
extern uint32_t app_ble_client_config_callback (const uint8_t rw, uint16_t handle, void *data, uint16_t size, uint16_t offset);

BT_GATTS_NEW_PRIMARY_SERVICE_16(_bt_if_dtp_primary_service, APP_BLE_SERVICE_UUID);

BT_GATTS_NEW_CHARC_16(_bt_if_dtp_char,
                      BT_GATT_CHARC_PROP_WRITE | BT_GATT_CHARC_PROP_INDICATE, APP_BLE_CHAR_VALUE_HANDLE, APP_BLE_CHAR_UUID);

BT_GATTS_NEW_CHARC_VALUE_CALLBACK(_bt_if_dtp_char_value, APP_BLE_CHAR_UUID128,
                                  BT_GATTS_REC_PERM_READABLE | BT_GATTS_REC_PERM_WRITABLE, app_ble_charc_value_callback);

BT_GATTS_NEW_CLIENT_CHARC_CONFIG(_bt_if_dtp_client_config,
                                 BT_GATTS_REC_PERM_READABLE | BT_GATTS_REC_PERM_WRITABLE,
                                 app_ble_client_config_callback);

static const bt_gatts_service_rec_t *_bt_if_ble_service_rec[] = {
...
static const bt_gatts_service_t _bt_if_ble_service = {
    .starting_handle = 0x0014,
    .ending_handle = 0x0017,
    .required_encryption_key_size = 0,
    .records = _bt_if_ble_service_rec
};
*/
...
static const bt_gatts_service_t * _bt_if_gatt_server[] = {
    ...
    //&_bt_if_ble_service, //0x0014-0x0017
    NULL
    };
